...
type (
	shortUrlsModel interface {
		...
		FindOneByCode(ctx context.Context, code string) (*ShortUrls, error)
...
func (m *defaultShortUrlsModel) FindOneByCode(ctx context.Context, code string) (*ShortUrls, error) {
	var resp ShortUrls
	query := fmt.Sprintf("select %s from %s where `code` = ? limit 1", shortUrlsRows, m.table)
...
func (m *defaultShortUrlsModel) Insert(ctx context.Context, data *ShortUrls) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Code, data.LongUrl)
	return ret, err
}
...
func (m *defaultShortUrlsModel) Update(ctx context.Context, newData *ShortUrls) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Code, newData.LongUrl, newData.Id)
	return err
}
